import cv2
import cvzone
from HandDetector import handDetector as HandDetector
    ...
    while True:
        ...
        if lmList:
            for button in buttonList:
                ...
                if (x < lmList[8][1] < x + w and y < lmList[8][2] < y + h) and x < lmList[12][1] < x + w and y < lmList[12][2] < y + h:
                    cv2.rectangle(img, button.pos, (x + w, y + h),
                                  (0, 255, 255), cv2.FILLED)
                    cv2.putText(img, button.text, (x + 20, y + 65),
                                cv2.FONT_HERSHEY_PLAIN, 4, (0, 0, 0), 4)
                    # l, _, _ = detector1.findDistance(8, 12, img, draw=False)
                    l, _, _ =detector.findDistance([lmList[8][1],lmList[8][2]], [lmList[12][1],lmList[12][2]], img)  # with draw
                    print(l)
                    # print(button.text)
